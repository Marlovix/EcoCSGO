...
class EquipmentCategoryRecyclerViewAdapter(
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        when (position) {
            0 -> adapter = values[EquipmentCategory.HEAVY]?.let { GunRecyclerViewAdapter(it, listener) }
            1 -> adapter = values[EquipmentCategory.SMG]?.let { GunRecyclerViewAdapter(it, listener) }
            2 -> adapter = values[EquipmentCategory.RIFLE]?.let { GunRecyclerViewAdapter(it, listener) }
        }
